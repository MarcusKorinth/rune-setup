...
- On native Windows runs, root-relative command and `cwd` spellings (`\tools\setup.exe`,
  `/tools/setup.exe`) are rejected at plan time: they inherit the caller's current drive and
  therefore cannot satisfy `${manifestDir}` determinism. Foreign-platform previews preserve
  these target spellings unchanged and cannot execute them.
