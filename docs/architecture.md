...
`rune validate` = stages 1–2 (fully static), followed by the environment-variable audit report (§4.3). `rune run --dry-run` = stages 1–4, rendering the plan's immutable sink projection (final argv with registered secrets masked, cwd, skip reasons, disabled inputs) and executing nothing. There is **no fake runner and no second plan**: the projection is a deterministic field-for-field copy of the one canonical plan, with only registered secret substrings and secret wrappers replaced. It performs no interpolation, condition evaluation, anchoring, or other execution decision, so dry-run and run cannot drift.

`ExecutionPlan` (deep-frozen, `readonly` types) has its own explicit `executionPlanVersion: 1`, independent of the manifest and result schema versions. It carries `manifestPath`, `manifestSha256`, `manifestSchemaVersion`, `platform`, `locale`, `preview`, `resolvedInputs` (secrets wrapped internally; disabled inputs carry their empty value plus `enabled`, `source`, and `ignored` state), `executionOptions: { failFast, logFile }`, and `steps: readonly PlannedStep[]` in declaration order. `executionOptions.logFile` is the effective absolute path after `--log-file` precedence and manifest-relative anchoring, or `null` when logging to a file is disabled. Internally, `PlannedStep` carries the real `ResolvedCommand` (secret-wrapped argv, cwd, env delta, timeout, success codes). The runner receives only this canonical plan. The `Session.plan()` facade, `RunStarted`, dry-run renderer, result construction, and IPC bridge receive one cached, engine-owned sink projection instead: structural fields are unchanged, while every free-form/value-bearing string is passed through the session's `SecretRegistry.mask()` and secret wrappers become `***`. This is masking, never a second execution plan.
...
**Output tail.** While a step is `RUNNING`, the Executor keeps a bounded ring buffer of its last 50 combined stdout/stderr lines (each tagged with its stream, already masked — lines enter the buffer from the same masked stream the events carry). If the step ends `FAILED` (including timeout), the buffer becomes its `outputTail` in the result file (§10), a possibly empty list of at most 50 masked lines; for `SUCCEEDED`, `SKIPPED`, `CANCELLED`, and `NOT_RUN` steps the buffer is discarded and `outputTail` is explicitly `null`. Full output stays in the log file; the tail exists for CI triage from the result alone.

**Counters.** The run result carries `stepsTotal`, `stepsExecuted`, `stepsSucceeded`, `stepsFailed`, `stepsCancelled`, `stepsSkipped`, `stepsNotRun`, and `nothingExecuted` (true iff `stepsExecuted == 0`). `stepsExecuted` counts every step that entered `RUNNING`, so `stepsExecuted = stepsSucceeded + stepsFailed + stepsCancelled` and `stepsTotal = stepsExecuted + stepsSkipped + stepsNotRun` hold for every outcome (`stepsCancelled` is 0 or 1 in v1 — at most one step is `RUNNING`). The generated result JSON Schema pins the locally representable constraints (non-negative integers and the `stepsCancelled` bound); `runResultSchema` additionally derives every counter from `steps[].state`, checks both arithmetic identities, `stepsTotal = steps.length`, the status/step-state combinations, and the `nothingExecuted` equivalence at parse time. These comparisons between sibling values are deliberately runtime-only because portable JSON Schema has no standard data-reference arithmetic; §14's counter tests pin both layers. A run in which every step was skipped is a **success** (exit 0, `status: "succeeded"`) — skipping is the authored outcome of conditions and platform blocks — but it is marked: `nothingExecuted: true` and a warning on stderr, so a pipeline that considers "nothing happened" suspicious can branch on the result without RUNE guessing intent. The warning is emitted only for real runs (`dryRun: false`): in a `planned` result (§10) the counters describe the plan — `stepsSkipped` from plan-time `SKIPPED`, `stepsExecuted = 0` — so `nothingExecuted` is always `true` there and is not warned about.
...
**Observer delivery contract:** run events are delivered synchronously, in order, from the engine's event-loop turn (observer callbacks are plain synchronous functions; the engine never awaits them). Observers must return quickly and must not throw; an observer exception is caught and swallowed — a broken renderer can never corrupt a run. `RunStarted` is first and `RunFinished` is last, exactly once each; no event is delivered after the `execute()` promise settles. The frontend's terminal event is withheld until every engine-owned sink has finalized. If that finalization fails after steps ran, the sole `RunFinished` carries the failure result with the executor's actual step states and counters; the facade then rejects with the corresponding `RuneError`. Observers can never influence execution.
...
| 1 | Execution failed: one or more steps failed or timed out (regardless of `failFast`), an execution-policy error occurred, or operational log-file I/O failed (RUNE-406) |
...
Versioned independently of the manifest schema (`resultSchemaVersion: 1`; `rune schema --result` emits its JSON Schema), written **atomically** (tmp + `fs.rename`) and on **every** outcome except usage errors — success, step failure, manifest error, input error, resolution/condition error, cancellation, internal error; only usage errors (exit 2, where no run was configured), writer crashes, and a hard crash of the process hosting the engine (exit 70 — under `--gui` the shell process, §9.4) skip it. An unknown run-pipeline throwable becomes a generic, secret-safe internal error while retaining its cause internally. Once result delivery starts, a sink failure escapes unchanged and is never retried; the last-resort CLI diagnostic is generic rather than echoing the throwable. Terminal human outcome and warning rendering occurs only after a requested result has been delivered successfully, so a writer crash leaves no false terminal summary. Result construction is engine-owned: the public `createFailureResult` factory accepts optional opened-session and completed-plan context, preserves resolved inputs, identity, locale and platform, and projects a completed plan as unchanged `SKIPPED` steps plus `NOT_RUN` steps for a real run or `PENDING` steps for a dry-run. A pre-execution log-file failure (RUNE-406) is the one execution error allowed to use that completed-plan projection; plan-time execution-policy failures such as RUNE-405 retain an empty step list rather than implying that a runnable plan completed. A sink failure discovered after execution instead preserves the completed run's real per-step states, output tails, and counters while reclassifying only its run-level outcome. Consequently, `status: "failed"` may contain no `FAILED` step for this documented run-level failure: log opening can leave executable steps `NOT_RUN`, while log writing or closing can fail after steps already reached `SUCCEEDED`. The real topology and counters remain authoritative, and a failed result still may not contain `CANCELLED` or `PENDING` steps. Hosts only capture that engine-produced terminal result, render session warnings once on failure, and call the public `writeResult`; they never duplicate result semantics. Run `status` maps to the exit code per the table below: every status implies exactly one exit code, and every exit code implies exactly one status once `dryRun` is known — exit 0 is `succeeded` for a real run and `planned` for `--dry-run`; every other code is unambiguous on its own. Consumers may branch on either, using `dryRun` to disambiguate exit 0.
...
| `failed` | 1 | one or more steps failed or timed out; an execution-policy error before planning completed (for example RUNE-405), which records an empty step list rather than inventing execution; or operational log-file I/O failed (RUNE-406), preserving the real `NOT_RUN`/`SKIPPED` or already-completed step topology |
...
- **per input** — `{id, value, source, secret, enabled, ignored}`: secret values always `null`; `source` records the supplying layer or is `null`; `enabled` records whether the input is active; disabled inputs use the type's empty `value`; `ignored` is always present as `"input disabled"` when a supplied value was ignored, otherwise `null` (with the supplying `source` retained)
- **per step** — `{id, title, state, exitCode, durationMs, command, skipReason, outputTail}`: `title` localized, `id` never; command arrays passed through the masker; `exitCode` is a number or `null`; `command` is `null` only for `SKIPPED`, `skipReason` is a string only for `SKIPPED`, and `outputTail` is an at-most-50-entry masked `{stream, line}` array only for `FAILED`, with all other non-applicable fields explicitly `null` (§7)
...
4. Dry-run renders the cached safe projection of the identical canonical plan execution consumes — all non-sensitive fields are identical; the projection only masks registered secret substrings and never evaluates, interpolates, anchors, or otherwise replans.
