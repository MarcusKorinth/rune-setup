...
`CancelToken` is a small class around a boolean flag plus an `AbortSignal`-style listener list (`cancel()`, `isCancelled`, `onCancel(listener)`); the engine is `async`/`await` on the Node event loop, so cancellation is cooperative and needs no threads. One execution-cancellation flow serves all frontends (GUI Cancel after Proceed → `rune.cancel` over the IPC bridge → token, CLI first `Ctrl+C`/SIGINT on every platform, headless POSIX SIGTERM, Windows `Ctrl+Break`/SIGBREAK). The Executor checks the token between steps; a live child process gets process-group/tree termination — POSIX: spawned with `detached: true`, `process.kill(-pid, 'SIGTERM')`, SIGKILL after 5 s grace; Windows: `taskkill /PID <pid> /T /F` (Node has no Job Objects; taskkill tree-kill is the standard). The interrupted step becomes `CANCELLED`, remaining steps `NOT_RUN`, run status `cancelled`, exit 6. Cancellation after planning but before execution instead uses `Session.describeCancelled()` (§9.1): it emits no run events, retains plan-time `SKIPPED` steps, and reports every `PENDING` step as `NOT_RUN`. Repeated SIGTERM on POSIX or SIGBREAK on Windows is idempotent and does not advance the SIGINT count; a second SIGINT force-exits. Programmatically sending SIGTERM on Windows terminates the target process unconditionally and cannot guarantee the ordinary result path. Step **timeout reuses the same kill path** with terminal state `FAILED` (RUNE-402). If the GUI shell's window is closed during a run, the shell's main process calls `Session.cancel()` and awaits `RunFinished` before exiting — the same path, never a bare abort of the engine.
...
export class Session {
  static open(manifestPath: string, options?: {
    ...
    mode?: RunMode;                        // frontend recorded in RunResult; default: non-interactive
  ...
  describeCancelled(): RunResult;                // after planning, before execute(); plan() must succeed;
                                                 // status `cancelled`, SKIPPED retained, PENDING → NOT_RUN;
                                                 // nothing executes and no run events are emitted
...
- **Methods:** `rune.open`, `rune.pendingInputs`, `rune.allInputs`, `rune.setValue`, `rune.plan`, `rune.describeCancelled`, `rune.execute`, `rune.cancel`, `rune.getStrings`, `rune.getThemeConfig` — every one returns a Promise. `rune.describeCancelled` is the pre-execution cancellation call: after a plan succeeds, Cancel before Proceed invokes it and receives the cancelled `RunResult` without run events; a window close in that phase makes main call `Session.describeCancelled()` directly. Before a plan can succeed, main writes the zero-counter cancellation result of §10. `rune.execute` is long-running: run events are pushed while it is in flight and its promise resolves with the `RunResult`; after it starts, Cancel uses `rune.cancel`, the only call serviced concurrently with it.
...
Fixed, identical on Windows and Linux — no `128+signal` arithmetic, so one pipeline script branches the same way everywhere. Each error class maps to exactly one code — the map is owned by `errors.ts` (`exitCodeFor`) alone; `cli/main.ts` applies the returned code, and the CLI's only other exit path is the documented second-SIGINT force quit (§7, §9.3), which cannot return through the ordinary code path (the GUI shell's main process uses the same `exitCodeFor` and `rune run --gui` forwards its code, §9.4); every code is reachable by a test. An unconditional externally sent Windows SIGTERM does not enter this ordinary result path and therefore cannot provide this exit-code contract.
...
│       ├── results/
│       │   ├── model.ts           # RunResult/StepResult (resultSchemaVersion 2): counters, outputTail, provenance
