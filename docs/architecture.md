...
- env = the one shallow-frozen parent-environment snapshot captured by the Executor immediately before `RunStarted` + interpolated `env:` overlay + reserved `RUNE_RUN_ID` / `RUNE_STEP_ID`; the same internal snapshot is used for every step and termination helper in the run and is never added to the `ExecutionPlan`, events, results, or root public API
...
- process-tree kill (cancellation, timeout, and output-stream failure, §7): POSIX children use `detached: true`. The runner sends `process.kill(-pid, 'SIGTERM')`; only `ESRCH` means the group is already absent, while `EPERM`, `EACCES`, and every other signal error are unconfirmed failures. After a sent SIGTERM it confirms that the group has no live members within 5 s, then sends SIGKILL; an absent group at SIGKILL is confirmed, a signal failure is not, and a sent SIGKILL gets a further bounded 5 s for confirmation. Each wait has a watchdog installed before its first probe, so a stuck probe cannot evade the deadline. Linux `/proc` probing ignores zombies and process-disappearance races (`ENOENT`/`ESRCH`); restricted or otherwise unclear scans conservatively treat the group as live. Windows spawns `<SystemRoot>\System32\taskkill.exe /PID <pid> /T /F` from the run's parent-environment snapshot, looking up `SystemRoot` case-insensitively and requiring a non-empty absolute value, with argv and `shell: false` — never through `PATH`, `WINDIR`, discovery, or a default. Only helper exit 0 confirms the Windows tree termination. On either platform, an unconfirmed operation makes one immediate best-effort direct-child SIGKILL when the child still has a PID and has not ended, then yields `terminationFailed`; waiting for child `close` is bounded to 5 s and cannot change that result. `terminationFailed` is fatal: the current step is `FAILED` with RUNE-401 and a null exit code, every later pending step is `NOT_RUN` regardless of `failFast`, and the run is failed. A confirmed operation preserves the original cause even if the bounded child-close wait expires.
