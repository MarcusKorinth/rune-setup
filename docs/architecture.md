...
- **Methods:** `rune.open`, `rune.pendingInputs`, `rune.allInputs`, `rune.setValue`, `rune.plan`, `rune.execute`, `rune.cancel`, `rune.getStrings`, `rune.getThemeConfig`, `rune.warnings` — every one returns a Promise and maps to one facade call. Two shell-lifecycle affordances ride the same transport without touching the engine: `rune.done` (the Result page's finished signal, so main can close with the run's exit code) maps to the window lifecycle, not the facade — it adds no engine behavior and so does not break invariant 11. `rune.execute` is long-running: run events are pushed while it is in flight and its promise resolves with the `RunResult`; `rune.cancel` is the only call serviced concurrently with it.
...
│       │                          #   failureResult/assertFailureExitCode host failure helpers
