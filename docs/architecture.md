...
export class Session {
  ...
  mask(text: string): string;                    // host-side sink masking; never reveals a secret
...
- **Methods:** `rune.open`, `rune.pendingInputs`, `rune.allInputs`, `rune.setValue`, `rune.plan`, `rune.execute`, `rune.cancel`, `rune.getStrings`, `rune.getThemeConfig`, `rune.warnings` — every one returns a Promise and maps to one facade call. `rune.plan` projects the frozen `ExecutionPlan` returned by `Session.plan()`; it never substitutes the dry-run `RunResult` returned by `Session.describe()`. Two shell-lifecycle affordances ride the same transport without touching the engine: `rune.done` (the Result page's finished signal, so main can close with the run's exit code) maps to the window lifecycle, not the facade — it adds no engine behavior and so does not break invariant 11. `rune.execute` is long-running: run events are pushed while it is in flight and its promise resolves with the `RunResult`; `rune.cancel` is the only call serviced concurrently with it.
...
2. **Manifest `gui:` block** (schema v1, §4.2) — `gui.accentColor`, `gui.logo` (window/taskbar icon and header logo), `gui.banner`, `gui.theme` (path to a CSS file), optional `gui.windowTitle`. Resolved by the engine (`getThemeConfig`, paths absolutized against `${manifestDir}`); the main-process bridge serializer projects asset paths to canonical `file:` URLs before the renderer applies them as variable overrides; presentation-only, ignored by CLI and non-interactive — no parity impact.
