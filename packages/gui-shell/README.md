...
Playwright-for-Electron smoke suite. After `npm ci --ignore-scripts`, prepare the runtime with
`npm run prepare:electron --workspace @rune/gui-shell`. Build it with `npm run build` from the
repository root and run its smoke suite with `npm run test:smoke --workspace @rune/gui-shell`.
